"""Result building helpers for the topic-modeling worker.
...
Used by:
- ``_compute_topic_payload`` in ``worker_tasks_topic`` for result building.
"""
...
def _build_topic_result_payload(
    *,
    ...
    Called by:
    - ``_compute_topic_payload`` in ``worker_tasks_topic`` for the run.
    """
    ...
    for topic in rust_topics:
        ...
        # Meaning-column slice -- respects the user's chosen display count so the
        # "Add to Workspace" parquet matches what they saw. The server-side label
        # fallback uses the same narrow slice for the same reason.
